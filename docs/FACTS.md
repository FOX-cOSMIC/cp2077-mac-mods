...
### F-053 — ❌ DECISIVE NEGATIVE: no tested ad-hoc re-sign entitlement combination unlocks inline `__TEXT` hooking — closes H-004/T-005 (open since old session 5) and refutes H-012 (2026-07-04)

- **Date:** 2026-07-04; empirical, in-vivo, 6 real launches of 6 distinct re-signed copies of the actual game binary (build 2.3.1, SHA256 `2a6ba5c2...` matching F-001 exactly — confirmed no drift before testing). Probe: `src/red4ext-mac/src/probes/h004_probe.cpp` (new, this session), a read-only diagnostic that fires at dyld image-load time, locates the game's `__TEXT` segment, attempts `mach_vm_protect()` and `mprotect()` to RWX on a page 1 MiB inside the segment, and — regardless of outcome — reverts immediately without ever writing a single byte. Launch wrapper: `tools/run-h004-probe.sh`.
- **Why this test:** H-004 ("inline `__TEXT` hooking remains blocked") had been open and unresolved since being carried forward from an old session on a different project/build (T-005, "smoke-test mprotect EPERM on current build," was never actually run). Separately, H-012 (this session, from reading `github.com/memaxo/RED4ext-macos`'s source) proposed that adding `allow-unsigned-executable-memory` + `allow-jit` might unlock it. `docs/reference/macos-codesigning.md` additionally asserted — without ever citing an empirical test — that `disable-executable-page-protection` was the entitlement that actually does this. None of these three claims had ever been tested against the current build. This fact resolves all of them at once.
- **Method — 6 isolated `.app` bundle copies** (never the live Steam install; each `ditto`'d fresh, SHA256-verified identical to the live binary before any change):
  1. **copy0 (control, untouched signature):** `mach_vm_protect`→`kr=2` (KERN_PROTECTION_FAILURE), `mprotect`→`errno=13` (EPERM). Expected baseline, now empirically confirmed on build 2.3.1 for the first time (closes T-005).
  2. **copyA (CDPR's original restricted entitlements preserved + `allow-unsigned-executable-memory`+`allow-jit` added, ad-hoc re-signed):** process **SIGKILL'd instantly (exit 137)**, no crash report. Root cause (isolated below): ad-hoc-signing while *preserving* CDPR's Apple-provisioned restricted entitlements (`com.apple.developer.sustained-execution`, `com.apple.application-identifier`, `com.apple.developer.team-identifier`) triggers an AMFI/kernel launch-time kill — those three keys require real Apple backing an ad-hoc signature can't provide. **This is a distinct, important gotcha, independent of the `__TEXT` question**, and explains why a naive "extract existing entitlements + add new ones" re-sign (the procedure `macos-codesigning.md` documents) can silently break launch entirely.
  3. **copyC (minimal re-sign: only `disable-library-validation` + `allow-dyld-environment-variables`, CDPR's restricted keys DROPPED):** launches clean (exit 0, no SIGKILL) — confirms the SIGKILL in copyA was caused by the preserved restricted keys, not by anything `__TEXT`-related. `mach_vm_protect`/`mprotect` both fail identically to copy0. Validates the whole re-sign→inject→probe pipeline works correctly when the restricted keys are excluded.
  4. **copyD (minimal + `allow-unsigned-executable-memory` + `allow-jit` — the exact H-012/memaxo claim):** launches clean. `mach_vm_protect`→`kr=2`, `mprotect`→`errno=13`. **Identical failure. H-012's specific claim is refuted** — matches this project's own (previously uncited) assertion in `macos-codesigning.md` that this pair does not grant `__TEXT` write access.
  5. **copyE (minimal + the pair + `disable-executable-page-protection`):** launches clean. `mach_vm_protect`→`kr=2`, `mprotect`→`errno=13`. **Identical failure — this REFUTES `macos-codesigning.md`'s own prior claim** that this entitlement "allows in-place modification of the game's `__TEXT`." That claim had never actually been tested; it is now known to be wrong as stated (at minimum, for ad-hoc signing).
  6. **copyF (minimal + the pair + `disable-executable-page-protection` + `get-task-allow`):** launches clean. `mach_vm_protect`→`kr=2`, `mprotect`→`errno=13`. **Identical failure.** Rules out the "AMFI only honors these entitlements alongside debugger-trust" hypothesis, at least for ad-hoc signing.
- **All four post-baseline variants (C/D/E/F) return `kr=2` (KERN_PROTECTION_FAILURE), never `kr=1` (KERN_INVALID_ADDRESS)** — confirming the test page is real, mapped, `__TEXT` in every case, and the *protection change itself* is what's being denied, not a bad test address. The negative result is a genuine kernel-level denial, not a methodology artifact.
- **Conclusion:** on this build, **ad-hoc code-signing (`--sign -`) cannot unlock `__TEXT` write access via ANY combination of entitlements tested**, including the one this project's own reference doc previously asserted would work. This corroborates (and now empirically confirms, rather than merely quotes) `memaxo/RED4ext-macos`'s own internal finding ("Even ad-hoc signed binaries with `disable-executable-page-protection` entitlement cannot modify existing code. The entitlements only work for newly allocated JIT memory, not existing code pages") — see the `reference_red4ext_macos_ports` note in project memory. The remaining untested lever is a **real Apple Developer ID signature** (not ad-hoc) — memaxo's own analysis calls this "probably not" sufficient either, but it has not been ruled out empirically by anyone, us included.
- **Independent, incidental finding (SIGKILL gotcha):** re-signing while preserving a publisher's *restricted*, Apple-provisioned entitlements (app-identifier / team-identifier / sustained-execution-style keys) under an ad-hoc signature causes an instant, silent SIGKILL with no crash report. Any future re-sign work (this project's or a fork's) must drop those keys, not merely "keep what was already present" as `macos-codesigning.md`'s Step 4 currently instructs.
- **How to re-verify:** `cmake --build build --target h004_probe`; `tools/run-h004-probe.sh` (defaults to the live install, read-only — safe to run anytime, makes no changes and only tests+reverts protection). To reproduce the re-sign variants, `ditto` a fresh `.app` copy and follow the minimal-entitlements recipe above (never preserve CDPR's restricted keys under ad-hoc signing).
- **Invalidates:** resolves **H-004** (status → resolved, negative) and **T-005** (closed). Refutes **H-012** in full (both the specific memaxo claim and, by extension, this project's own `disable-executable-page-protection` claim). Corrects an uncited, untested assertion in `docs/reference/macos-codesigning.md` (annotated there). Does not change FA-001's core conclusion (`__TEXT` is immutable under stock signing) — extends it to show re-signing ad-hoc doesn't fix it either, at least not via these entitlements.
...
- **The post-open chain (all real code, macOS binary):** the F-049 resolver's validate/read step `FUN_103d9622c` fetches the **RedScriptsHost** singleton (`FUN_10223a2ac` → `&DAT_107d7e4c8`, vtable `PTR_FUN_1070012c0`) and calls its **`vtable+8` = `FUN_10223a748`** — a ~290-line loader that: (1) **parses** the blob via `FUN_1021f0204` (reads the file through the FS object `DAT_1090031f8` `vtable+0x10`), erroring "Failed to load scripts" on parse failure; (2) **validates** via `FUN_1021fbf90`; (3) **registers** the parsed definitions — sorts hash→pointer maps at `host+0x30`/`host+0x40`, calls `FUN_102237ae8` per entry, invokes each definition's `vtable+0xe8`, and **commits** them into the RTTI/script VM via `FUN_1021e8900`/`FUN_1021e897c`. This is a complete, working loader — **no macOS short-circuit/stub in the load or register path.**
- **The validator is a real per-type RTTI cross-check.** `FUN_1021fbf90` iterates every parsed definition, dispatches by kind (`plVar8->vtable+0x18` kind selector: class/function/enum/etc. → `FUN_1021fc1a4` / `FUN_1021fc290` / `FUN_1021fc47c` / `FUN_1021fc61c` / `FUN_1021fcee0`), each checking the definition against the game's live native types. It tallies failures and, if any, formats **"Validation failed for %u type(s)"** and reports it through the diagnostics callback (`param_3` `vtable+0x10`), returning 0 (→ the loader skips registration).
- **Only the in-process COMPILER is stubbed:** `FUN_103d963a4` (the `ShouldCompileScripts`/compile call inside the F-048 resolver) is literally `return 0;`. Correct by design — compilation is done externally by the shipped native `scc` (F-046), so the engine only ever *loads* the pre-built blob.
- **Consequence — the two (and only two) failure modes, both silent on macOS:** (a) blob **parse** failure → "Failed to load scripts"; (b) **validation** failure (a definition references a native type/method that doesn't match this build) → "Validation failed for N types". Both messages go to the diagnostics/error callback — the **same sink that has no visible output without RED4ext** (per the F-050 follow-up: game stdout and unified log show zero script chatter). So if redscript is silently not taking effect, it is one of these two, not missing infrastructure.
- **Prediction:** since the user's `scc` is the *native toolchain shipped with this exact build* (F-046), the compiled blob should validate against the build's own RTTI → **redscript should load and execute.** The pending in-game money test (bean q90k) is the empirical confirmation; if it fails, the cause is `FUN_1021fbf90` validation (next step: surface that error sink, e.g. by hooking the diagnostics callback or dumping the "Validation failed" path).
- **How to re-verify:** scratchpad scripts `trace_postopen.py`, `trace_redscriptshost_vt.py`, `trace_validator.py` against project `cp2077-v2` / program `Cyberpunk2077` — chain `FUN_103d9622c` → `FUN_10223a2ac` → RedScriptsHost vtable `PTR_FUN_1070012c0` slot +8 `FUN_10223a748` → `FUN_1021f0204` (parse) / `FUN_1021fbf90` (validate) / `FUN_1021e897c` (commit); confirm `FUN_103d963a4` == `return 0`.
- **Invalidates:** nothing. Extends F-050 (load confirmed) and F-049 (resolver) with the downstream register/link path; narrows bean q90k's "if unchanged" branch to a single function (`FUN_1021fbf90`).

### F-050 — ✅ EMPIRICALLY CONFIRMED: the macOS game DOES open `r6/cache/final.redscripts` at boot — F-047's "never loads at runtime" was an `lsof` false-negative (2026-07-02)

- **Date:** 2026-07-02; **method: `sudo fs_usage -w -f filesys` continuous syscall trace** over a normal Steam launch to main menu, build 2.3.1. Trace log `/tmp/fs_usage_redscript.log` (world-readable). Re-run via `scratchpad/trace_redscript_boot.sh`.
- **DECISIVE POSITIVE — the game process (`Cyberpunk2077`, PID 764661, the same PID that mmaps `tweakdb_ep1.bin`, so unmistakably the engine) did exactly this, in order, in ~60µs:**
  ```
  stat64        r6/cache/final.redscripts            (success)
  stat64  [2]   engine/cache/final.redscripts        (ENOENT — the fallback path)
  open F=113 (R____)  r6/cache/final.redscripts       (opened read-only, fd 113)
  ```
- **This is precisely the F-049 resolver behavior made real:** Primary blob path `r6/cache/final.redscripts` (exists → opened) + Fallback `engine/cache/final.redscripts` (missing → ENOENT), via the raw `stat()`/`open()` the static trace predicted. No explicit `RdData` lines for the fd because a 15 MB blob is **mmap'd** (like the base archives, F-045), not `read()`-streamed — the `open`+mmap IS the load.
- **Why F-047 got the opposite answer (the correction):** F-047 relied on **`lsof`** — an immediate snapshot plus 0.5s-interval polling. The blob open→mmap→close is a **single transient event lasting well under one poll interval**, so `lsof` structurally could not see it (it lists only fds open *at the instant of each poll*). `fs_usage`, a continuous kernel syscall trace, captured it on the first try. F-047's "definitive negative" was a measurement artifact, not a real absence.
- **What this DOES prove:** the compiled redscript blob produced by `scc` (F-046) **is stat'd and opened by the shipping macOS engine at boot** — the loader is alive, path-based (F-048), reached at boot through the subsystem-init chain (F-049), in the default (empty-`obj+0x150`) configuration. redscript is **not** blocked on a Phase-5 hook to reach the loader.
- **What this does NOT yet prove:** that the loaded bytecode is linked into the script VM and its `@wrapMethod`/`@replaceMethod` hooks actually execute in gameplay. "Opened + mmap'd" ≠ "validated, linked, and running." F-047's *only other* observation — a test `.reds` on-screen message never appeared — now has an unknown cause that is **no longer "the file isn't loaded"**; the likelier candidates are a flawed test mod (wrong `@wrapMethod` target, or an on-screen-message API that doesn't render) or a post-load validation/version gate rejecting the blob silently.
- **Next experiment (cheap, decisive, needs a fresh mod — the old `MacRedscriptTest` was cleaned up):** author a *minimal, robustly-observable* `.reds` (e.g. `@wrapMethod` a frequently-called, low-risk method to write a sentinel to a file the game can reach, or a guaranteed-visible UI change), `scc`-compile via `launch_modded.sh`, and check the sentinel in-game. If it fires → **redscript works natively on macOS** (a major project win, re-sequences Phase 6 far earlier). If it doesn't → trace the post-open path (blob validation/version check / VM registration) as the real gate.
- **How to re-verify:** `sudo bash scratchpad/trace_redscript_boot.sh`, play to main menu, ENTER; grep the log for `Cyberpunk2077.*final.redscripts` (filter out `mds`/`mdworker`/`fseventsd`, which are Spotlight re-indexing the freshly-written blob and are NOT the game).
- **Invalidates:** **F-047's central claim** ("the game NEVER loads `r6/cache/final.redscripts` at runtime") — annotated there. Confirms F-049's empirical prediction (a boot-time `stat` of the blob) and its candidate-(a) explanation (lsof missed the transient open). Does not disturb F-048's path-based-not-depot finding (further confirmed: a literal filesystem `open`).

### F-049 — The redscript blob resolver IS reached at boot via a raw `stat()`, gated by an empty-string check — correcting F-048's caller analysis; the remaining mystery is now empirical, not structural (2026-07-02)

- **Date:** 2026-07-02; Ghidra (read-only, headless), decompiler-confirmed, macOS `Cyberpunk2077` build 2.3.1. Corrects the caller analysis in F-048 (whose core claim — path-based, not archive-depot — is reconfirmed and strengthened here).
- **The resolve worker is a literal `stat(2)` syscall.** The engine-file object at `DAT_1090031f8` (a `red::PoolEngine` allocation, ctor `FUN_10170e308`, vtable `PTR_FUN_106f501c0`) has `vtable+0xc0` = `FUN_10170e298` = "return `this+8`" (its base-path string) and `vtable+0x38` = `FUN_10170e84c` → worker `FUN_101712a3c`, which is exactly `iVar=_stat(path,&st); return iVar? 0 : (st.st_mtimespec.tv_sec/100 - 0x19db1ded53e8000)` — i.e. **get-modified-time via a real filesystem `stat()`, 0 if missing.** This *disproves* the earlier "depot-registry lookup with no syscall" idea: a blob lookup is a genuine FS stat that `fs_usage` would capture.
- **There is a BOOT-TIME path to the resolver (F-048 said there wasn't).** `FUN_1035f0a08` (engine/framework init; it reads `engine+0x308` = framework, the F-037 layout) unconditionally calls `FUN_103d9e494`, the subsystem bring-up that initializes Network → ViewportManager → GameServices → LocalizationManager → **Scripts** → ContentManager → Renderer → InkSystem → DlcManifests. Its Scripts phase calls `FUN_103d99e44`, which reaches the resolver. This runs during normal game startup, independent of the per-frame hot-reload watcher.
- **The gate is an empty-string test, and EMPTY selects the full builder.** `FUN_103d99e44` routes on `FUN_10002b9d4(obj+0x150)`, which is `(*(uint*)(obj+0x150+0x14) & 0x3fffffff) == 0` — true iff the RED-engine `std::string` at `obj+0x150` has length 0. **Empty → `FUN_103d9a028`** (the full builder: selects the variant filename `master`/`final`/`profiling`/`final_noopts`.redscripts, joins the base dir, applies the `scriptsBlobPath` override `DAT_1090dbe00`, `stat()`s primary+fallback, and — when `DAT_1090dbeb1`≠0 — compiles via `FUN_103d963a4`). Non-empty → the thin re-stat `FUN_103d99eac`. Since `obj+0x150` is empty by default, retail boot takes the **full builder**.
- **The resulting contradiction (now the open question):** the full builder runs at boot and `stat()`s `master`/`final.redscripts`, which is a real syscall — yet F-047's `lsof` + `fs_usage` traces recorded **zero** access to any redscript path across the whole startup→menu→load window. Something breaks the chain between "resolver entered" and "path stat'd": candidates — (a) F-047's trace watched `open()` and did not surface `stat()`/`getattrlist()`; (b) `obj+0x150` is *not* empty at boot (pre-seeded), diverting to `FUN_103d99eac` which stats a different/empty path; (c) `FUN_103d9e494`'s Scripts phase early-returns on this build before `FUN_103d9a028`; (d) the base-dir string (`obj+8`) is empty/wrong so the built path isn't under `r6/cache`. Distinguishing these is **empirical**, not more static RE.
- **Sharpened next experiment (cheap, decisive):** `sudo fs_usage -w -f filesys "Cyberpunk2077" | grep -iE 'redscripts|r6/cache'` capturing **`stat`/`getattrlist`/`lstat`, not just `open`**, over a cold boot to main menu. If a `stat` of `master.redscripts` or `final.redscripts` appears → the resolver runs and the failure is downstream (validate/compile/VM-register); if none appears → the Scripts phase is short-circuited before `FUN_103d9a028` and that short-circuit (or the `obj+0x150` seeding) is the hook target.
- **How to re-verify:** scratchpad scripts `trace_watcher_callers.py`, `trace_host_vtable.py`, `trace_depot_vtable.py`, `trace_boot_and_worker.py`, `trace_gate.py` against project `cp2077-v2` / program `Cyberpunk2077` (chain: `FUN_1035f0a08` → `FUN_103d9e494` → `FUN_103d99e44` → gate `FUN_10002b9d4` → `FUN_103d9a028`; worker `FUN_101712a3c` = `_stat`; vtable `PTR_FUN_106f501c0` slots +0x38/+0xc0).
- **Invalidates:** the "ONLY callers are the reload watcher / never executes at boot" bullet of **F-048** (annotated inline there). Does NOT invalidate F-048's path-based-not-depot conclusion (reconfirmed) or F-047's runtime-negative observation (reconfirmed; now needs a stat-level re-trace to explain).

### F-048 — The macOS script-blob loader is filesystem-PATH-based (not archive-depot), and its only callers are the per-frame hot-reload watcher, gated behind a reload-dirty flag (2026-07-02)

- **Date:** 2026-07-02; Ghidra (read-only, headless), decompiler-confirmed on the shipping `Cyberpunk2077` macOS binary (build 2.3.1).
- **Refines F-047.** F-047 proved the blob is never opened at runtime and left open *whether* the Mac build even loads scripts from the loose `r6/cache/*.redscripts` file or bakes them into `.archive` depots (the standing hypothesis, by analogy to F-045). **The depot hypothesis is disproven: script-blob loading on macOS is genuinely filesystem-path-based.**
- **The Engine/Scripts settings struct** (singleton `DAT_108943550`, getter `FUN_103d9be38`, schema registered by `FUN_103d9c648`) holds the string fields `PrimaryBlobPath`, `FallbackBlobPath`, `PrimaryBlobModifiedDate`, `FallbackBlobModifiedDate`, `BlobPathUsed` (a status string set to `"Loaded"`/`"None"`), `ShouldCompileScripts`, `CompileScriptsSuccess`.
- **The resolver** (`FUN_103d9a028`, and the sibling `FUN_103d9a8dc`; thin re-validate variants `FUN_103d99eac`/`FUN_103d9a74c`) does, in order:
  1. **Selects the blob filename by build variant:** `"master.redscripts"` (default), `"final.redscripts"`, `"profiling.redscripts"`, or `"final_noopts.redscripts"`.
  2. **Builds a full path** = base dir from the FS-abstraction object `DAT_1090031f8` (`vtable+0xc0`) joined (`FUN_10344993c`) with the filename, plus an override string global `DAT_1090dbe00` (this is the sink that the `scriptsBlobPath` argv key from `FUN_103d8c188` feeds).
  3. **Probes each path via `DAT_1090031f8` `vtable+0x38`** (returns a modified-date/handle; **0 = file absent**) and records it into `Primary/FallbackBlobModifiedDate`. This is an ordinary path stat through an FS-abstraction vtable — **not** a content-key lookup into an archive depot.
  4. Gated by `DAT_1090dbeb1 != 0`, compiles via `FUN_103d963a4` and records `CompileScriptsSuccess`; validates/reads the blob via `FUN_103d9622c` only when the stored modified-date changed.
- **The resolver's ONLY callers are the per-frame scripts hot-reload watcher** `FUN_103d996e0` and `FUN_103d99938` (both use `_mach_absolute_time()` frame timing, bump a frame counter at `param+0x28`, and `FUN_103d99938` kicks a `"StartDataReload"` job). Both invoke the resolver **only when a reload-dirty flag is set** — `*(param+0x57) != 0` (reload enabled) AND `param[0xb] != 0` (dirty, consumed to 0 on read). In a retail run where that dirty flag is never raised, the resolver — and therefore the loose-blob stat/compile/load path — never executes. No separate boot-time caller of this settings-struct resolver appears in the reference graph.
  - **⚠ CORRECTED 2026-07-02 by F-049:** this bullet is WRONG. There **is** a boot-time caller — `FUN_1035f0a08` (engine/framework init, dereferences `engine+0x308`=framework) → `FUN_103d9e494` (subsystem bring-up: Network/Viewport/GameServices/Localization/**Scripts**/ContentManager/Renderer/InkSystem) → `FUN_103d99e44` → the resolver. And it is NOT reload-dirty-gated: `FUN_103d99e44` routes on `FUN_10002b9d4(obj+0x150)` = "is the `std::string` at obj+0x150 EMPTY" (`(*(uint*)(obj+0x164)&0x3fffffff)==0`); **empty (the retail default) → the FULL builder `FUN_103d9a028`** that constructs & `stat()`s `master`/`final.redscripts`. So the resolver runs at boot. See F-049.
- **Consistency with prior empirical tests:** this explains why the earlier `-scriptsBlobPath <abs path>` launch and the `master.redscripts` copy tests (F-047 / bean `cp2077-mac-mods-4paj`) produced no observable effect — the override global `DAT_1090dbe00` is only consumed inside the reload-gated resolver, which did not run.
- **Remaining gap (now a hypothesis, not a fact):** whether the game's *initial* script load at boot uses a different code path entirely (not this Engine/Scripts settings resolver) is not yet established; if it does, that path — or the dirty-flag trigger — is the real hook target for making redscript reach gameplay.
- **Implication for the roadmap:** redscript still needs a Phase-5-class intervention (a hook), consistent with F-047 — but the lever is now specific: either raise the reload-dirty flag (`param+0x57`/`param[0xb]`) so the existing path-based resolver runs, or hook the boot-time loader once located. It is *not* an archive-repack problem.
  - **⚠ SUPERSEDED 2026-07-02 by F-049:** the boot-time loader is now located (`FUN_103d9a028`, reached at boot per above), and the resolve worker is a raw `_stat()` — so the puzzle is no longer "find the loader" but the *empirical* contradiction in F-049 (a raw `stat()` of `master`/`final.redscripts` should occur at boot, yet F-047's trace saw none). Next step is empirical (stat-focused boot trace), not necessarily a hook.
- **How to re-verify:** Ghidra headless against project `cp2077-v2` (program `Cyberpunk2077`) with the scratchpad scripts `trace_blob_callers.py` → `trace_getter_callers.py` → `trace_resolver_callers.py` (find callers of `FUN_103d9c648` → getter `FUN_103d9be38` → resolvers `FUN_103d9a028`/`FUN_103d9a8dc`/`FUN_103d99eac`/`FUN_103d9a74c`); read the decompiles for the variant-filename strings, the `vtable+0xc0`/`vtable+0x38` calls on `DAT_1090031f8`, and the `*(param+0x57)`/`param[0xb]` gate in `FUN_103d996e0`/`FUN_103d99938`.
- **Invalidates:** the "scripts baked into `.archive` containers / resource-depot load" hypothesis for the macOS build (none — it was a hypothesis, not a prior fact). Extends F-047.
...
- **⚠ CORRECTED 2026-07-02 by F-050:** the "Runtime: DEFINITIVE NEGATIVE" and "Conclusion" bullets below are WRONG. A `sudo fs_usage` continuous syscall trace caught the game process `stat64`+`open`ing `r6/cache/final.redscripts` (fd 113) at boot. The `lsof` method used here is a *snapshot* tool (immediate + 0.5s polling) that cannot see a single transient open→mmap→close; it produced a false negative. **The game DOES load the blob.** What remains genuinely unverified is only whether the loaded bytecode *executes* (the on-screen message not appearing may be a flawed test mod, not an unloaded blob). See F-050. The static "Notable wrinkle" reasoning below (dead code / gated / short-circuited) is therefore also moot — none of those hold; the path runs.
...
- **Mechanism (NOT the TweakDB path — a direct live-memory write):** `DoStatProbeCells` RAW-INT mode (`TWEAKXL_POKE_PROBE_CELLS=1`, signal `i310915`) scans all writable private VM regions for the int32 `310915`, finds 10 addresses, writes a DISTINCT marker (`400+i`) to each + a re-apply thread, and the user reads the changed value off-screen to identify the authoritative cell. Marker `400` (idx 0, **address `0x31606a7d8`**) was the one the money UI displayed → that cell is the live eddies store. The other 9 were mirrors/duplicates. `RevertMarkers` restored the originals cleanly.
- **WHY MONEY AND NOT RAM/HP:** money is a **stored counter** — the game must hold your exact eddies as a literal value, so it exists in memory verbatim and is writable. The displayed RAM cap (32) and max health (322) are **derived stats** computed from base+modifiers at read time and **never materialized as their displayed integer** — value-scans for `32` and `322` (and `±0.6` tolerance) found 0 matching cells in any player container (the container also keys entries by hashed runtime ids, not the raw enum). So derived/displayed stats are not reachable by value-scan; stored counters are. (Failed RAM/HP attempts: FA-017, FA-018, FA-019.)
- **What this unblocks:** the **live-memory-write path reaches gameplay**, end to end (scan → write → re-apply → visible), with no crash — using only `mach_vm_read_overwrite`/`mach_vm_write` and structural/value scans, no `gameInstance` root (sidesteps F-038). This is the route to visible changes that the TweakDB-flat path (F-043/FA-013…FA-016) could not deliver on an existing save.
- **Crash-safety:** the write is a single 4-byte `SafeWrite` into a value the engine itself stores; RAW-INT money values are rare enough to yield ~10 candidates (not the thousands the int-`32` scattershot hit), and we mark+read to identify rather than blast — no crash (contrast FA-014).
- **How to re-verify:** `/tmp/run_probecells.sh` (`TWEAKXL_POKE_PROBE_CELLS=1 TWEAKXL_POKE_GO_FILE=/tmp/red4ext_go`); in-world, `echo i<eddies> > /tmp/red4ext_go`; read money = `400+index`; `echo 0 > /tmp/red4ext_go` to revert. Log tag `[probe-cells]` in `/tmp/red4ext-mac.log`.
- **Invalidates:** the session-long premise that a visible change required solving the TweakDB→gameplay propagation wall — a *direct live write* gets there independently. Does NOT resolve the TweakDB-flat wall (F-043/FA-016 stand for flat-driven changes); reframes the goal as achieved via the live-write path.

### F-043 — In-place flat edits REACH live engine state (crash proves it); safe-repoint edits do NOT; the wall = interning + wrong targets (2026-06-03)
- **Evidence:** the in-place x10 run is the ONLY crash in the machine's history (game ran ~12× this session, zero crashes, crashed only on the in-place edit run). Crash = `EXC_BREAKPOINT`/`BRK #1` (esr 0xf2000001, EC 0x3C), thread redDispatcher9, faulting pc static `0x10002432c`.
- **Backtrace decompiled (6-agent workflow over `reference/ghidra/Cyberpunk2077.c`):** `0x10002432c` ∈ `FUN_100024298` = the engine's **memory-allocator OOM / pool-exhaustion abort** (packs ASCII "Pool" tag 0x6c6f6f50, resolves pool/allocator names via `FUN_10001b5b4`/`FUN_100018f78` — "DefaultAllocator"/"FrameAllocator"/…, args = requested size+alignment, then `SoftwareBreakpoint(1,…)`). The "recursive core" `0x102172xxx/0x102173xxx` = RedEngine **RTTI property serialization walker** (FNV-1a CName, "%hs.%hs" paths; no float/stat math). Repeated callee `0x10225xxx` = **REDscript bytecode VM**. NONE of these read TweakDB flats/stats — the trap is an allocation failure, NOT a stat range-assert.
- **Root cause:** the **boost** ×10'd `BaseStatPools.*.initialValue` — a **percentage** (base 100, F-039) that entity stat-pool init genuinely consumes — producing an absurd derived magnitude that blew an allocation size/count → pool exhaustion → trap. Interning (F-031: pooled FlatValues, zero-slack buffer) dragged the shared `-1e7/-1e8` `BaseStats.*.max` sentinels along (observed cascade: initialValue 1e5→1e6→1e7→1e8).
- **THE KEY RESULT:** in-place edits (`EditScalarFlatInPlace`, overwrite bytes at the original FlatValue buffer location) **reach live engine/gameplay state** — demonstrated negatively by the crash. The interning-safe repoint (`EditScalarFlatSafe`: new FlatValue + repoint the +0x40 entry's tdbOffset) is **GetFlat-visible but gameplay-invisible** (gameplay holds a pointer to the ORIGINAL location), which is why every safe edit this session (HP/RAM/prices) verified via GetFlat yet showed nothing. The wall was (a) interning making in-place a shotgun + (b) target selection (sentinels/percentages/computed values).
- **Path forward:** in-place edit of a REAL on-screen Float (NOT a sentinel/percentage/size-feeding value), modest multiplier, small batch, mind interning collateral. Vendor item **price** is the prime target (stored Float, live-read at shop-open, on-screen). HP/RAM stay out of scope for scalar edits (computed per-entity F-039). Price-only in-place x10 should NOT OOM (prices don't feed allocation sizes).
- **How to re-verify:** crash report `Cyberpunk2077-2026-06-03-144927.ips`; workflow `crash-rootcause` (run wf_9785a1d0-10a). Re-test: `TWEAKXL_PRICE_X10=1` alone (no `TWEAKXL_BOOST_STATS`).
- **Invalidates:** the implicit assumption that GetFlat-verification implies gameplay-visibility (F-036 already hinted; this nails the mechanism). Corrects this session's earlier mislabel of the crash as a "stat range assert" — it is an allocator OOM.

### F-042 — Full record CREATE + array APPEND works: DoubleRam.yaml replicated in memory, game GetRecord confirms (2026-06-03)
- **Context:** the answer-key RAM mod (DoubleRam.yaml) = create `Character.PlayerDoubleRam_inline0` (ConstantStatModifier, statType=BaseStats.Memory, modifierType=AdditiveMultiplier, value=0.5) + `!append-once` onto `Character.Player_Stat_Pools_Modifier.statModifiers`. This validates that exact op sequence in-vivo (no save), the foundation for the applicator's CreateRecord + AppendOnce.
- **Constants confirmed (in-vivo):** `ConstantStatModifier` **baseHash = `0xe22dd781`** (factory `&0x1f`=1); the record's RTTI type-name CName `0xbcbb0d900b0e3ed0` (= `gamedataConstantStatModifier_Record`) sits at **CClass+0x18** (GetNativeType = record vtable+0x08, F-034). Typed-flat FlatValue vtables (static, base 0x100000000): Float `0x106e925f0`, Int32 `0x106e926f8`, Bool `0x106e92d78`, TweakDBID `0x106e9b448`, **CName `0x106e93198`**. CName value = FNV1a64(name) (e.g. `AdditiveMultiplier`=`0x425a5b5b0d67f8c6`); ForeignKey/statType value = the 8-byte target TweakDBID (e.g. `BaseStats.Memory`=`0x4c58e91c` len 16).
- **DynArray-flat memory layout (for AppendOnce):** the flat's value at `flatDataBuffer + tdbOffset` is `[vtable@+0x00][data ptr@+0x08][size u32@+0x10][capacity u32@+0x14]` (a `red::DynArray<TweakDBID>`). `Character.Player_Stat_Pools_Modifier.statModifiers` had size==cap==121 (zero slack) → append = realloc element buffer (size+slack), copy + write new id, repoint `{data@+0x08, size@+0x10, cap@+0x14}`. (The earlier `data@+0,count@+0xc` reading was wrong — that hit the FlatValue vtable.)
- **CreateTDBRecord is callable (F-031 confirmed live):** `((void(*)(TweakDB*,uint32 baseHash,uint64 idRaw))StaticToRuntime(0x1026b8db8))(db, 0xe22dd781, recIdRaw)` — builds the record from the `<recordId>.<prop>` flats (which must already be in +0x40, F-041) AND inserts into recordsByID(+0x58)/recordsByType(+0x88). **Returned with no crash; `GetRecord` then resolves the new record** (M, vtable matches ConstantStatModifier cluster, `CallGetBaseHash`=0xe22dd781).
- **GROUND TRUTH (in-vivo, build 2.3.1, title screen, `TWEAKXL_TEST_CREATE_RECORD=1`):** created the 3 flats → CreateTDBRecord → `GetRecord(Character.PlayerDoubleRam_inline0)` non-null, baseHash OK → appended to statModifiers (size 121→122, our id is the last element) → `UpdateRecord(Player_Stat_Pools_Modifier)=1` → **`CREATE+APPEND=OK`**. No crash; game ran to title.
- **Significance:** first end-to-end proof macOS can CREATE records + MUTATE arrays in TweakDB — the two ops the entire stat-mod ecosystem needs and our applicator lacked. The DoubleRam mod's effect is reproduced at the memory level. Remaining: (1) confirm the in-game effect (load a save → RAM bar) since the apply fires at TweakDB-init before entity stat-init (F-018/F-030); (2) wire CreateRecord/AppendOnce into the applicator so it runs the actual YAML (currently a hardcoded probe).
- **How to re-verify:** `TWEAKXL_TEST_CREATE_RECORD=1`; grep `[rec-create] ... CREATE+APPEND=OK`.
- **Invalidates:** the applicator-skips-everything-but-Assign limitation as a hard wall — the underlying primitives now exist (F-041 + this).

### F-041 — NEW-flat creation works: insert a new key into the +0x40 SortedUniqueArray, the game's own GetFlat resolves it (2026-06-03)
- **Context:** the answer-key RAM mod (DoubleRam.yaml, F-040) creates a record + appends it — ops needing brand-new flats/records, which we'd never made (only EDITED existing). This validates the foundational new-flat primitive in isolation, no save.
- **Mechanism (`CreateFlat`, `TweakDB.cpp`):** `AppendFlatValue` allocates the FlatValue in the +0x148 buffer (growing it; F-031), then insert the 8-byte key `{nameHash,nameLength,tdbOffset(24-bit BE)}` into the db+0x40 SortedUniqueArray at its sorted position. The array had **size==capacity==3,306,462 (ZERO slack, confirming F-031)** → we allocate a NEW entries array (size+1024 slack) via `posix_memalign`, copy old+inserted in sorted order, and repoint +0x40 `{entries@+0x00, capacity@+0x08, size@+0x0c}`. Never memmoves the live array in place (same move-don't-mutate model as `GrowFlatBuffer`); old array is game-owned and left intact (our replacement leaks — fine, one-time at load).
- **GROUND TRUTH (in-vivo, build 2.3.1, title screen):** created `TweakXLMac.ProbeFlatV1.value` (absent pre-insert) as Float 0.5 → inserted @pos 2459637, tdbOff 0x40a8b8 → **the game's own `GetFlat` (`FUN_102b76708`) returns fv with the Float vtable and `value=0.5`** (`GAME-SEES-NEW-FLAT=YES`). No crash; game continued to title. Our `ResolveFlatOffset` also HIT.
- **Significance:** first proof macOS can ADD to TweakDB, not just edit. Unblocks `CreateRecord` (the game's `CreateTDBRecord` reads `<recordId>.<prop>` flats via GetFlat — they must exist in +0x40 first). The +0x40 sorted insert + repoint is safe at our load-time apply point (flats array is static post-`FUN_102b75744`, F-018).
- **How to re-verify:** `TWEAKXL_TEST_CREATE_FLAT=1`; grep `[flat-new] ... GAME-SEES-NEW-FLAT=YES`.
- **Invalidates:** none. Builds on F-031 (flats-array layout) / F-036 (game-GetFlat as ground truth).
...
### F-039 — Player HP/RAM magnitude lives in StatModifierGroup → ConstantStatModifier.value, NOT on the Stat/Pool record (2026-06-02)
- **Stat model (gibbed schema, now at `reference/gibbed-schema/`):** `Stat`{enumComment, localizedDescription, localizedName, enumName, localizedStatDisplay, improvementRelation, flags[], substats[], min, max}; `StatPool`{enumComment, stat→Stat, regen, decay, enumName, initialValue, enableDefeated}; `StatModifierGroup`{statModsLimitModifier, statModifiers[]→StatModifier, statModsLimit, drawBasedOnStatType}; `StatModifier`(abstract){statType→Stat, modifierType(Name)} + derived `ConstantStatModifier` adds `.value`(Float). **A Stat has NO modifier array** — modifiers are applied via StatModifierGroups at the entity/character level.
- **Runtime confirmation (DumpStatModifiers, `TWEAKXL_DUMP_STAT_MODIFIERS=1`):** materialized record object field offsets — Stat `BaseStats.Health`: `.localizedDescription`@+0x48, `.localizedStatDisplay`@+0x54, `.enumName`@+0x60 (= CName "Health"). StatPool `Player_Health_Base`: `.enumComment`@+0x48, `.stat`@+0x54 (→ value = `BaseStats.Health` id 0x10·68effe3a), `.regen`@+0x60. NONE expose a base-HP/RAM float. The pool `.initialValue`=100 is a percentage (prior finding).
- **Seeder reverse-index @ record+0x288:** `FUN_10098fea0(statTypeId)` = GetRecord+typecheck → the Stat record; `FUN_102ac0c70/FUN_102ac0a90` read the modifier-source array at **stat-record+0x288** (a runtime-built "which groups affect this stat" index; expected empty/sparse at title screen before any entity registers). The Windows-style modifier arrays the seeder iterates (`local_b0+0x54/+0x60`) are on the GROUP objects reached via +0x288, not on the Stat record.
- **Accessors/layout reused:** GetRecord live = `FUN_102b745d0`; the field/array resolver `FUN_100a261ac` = `flatDataBuffer + TdbOffsetOf(field)` where TdbOffset = field bytes [5..7] big-endian (matches our `TdbOffsetOf`). So any record flat-field (8 bytes) resolves to its FlatValue/DynArray by the same +0x148-buffer + tdbOffset path.
- **CRC32 naming VERIFIED:** `Crc32("BaseStatPools.Player_Health_Base.stat") = 0xedd1dc1b` exactly matches the live record field hash → `MakeCompactId` is correct and named flats resolve; with the gibbed vocab we can name+author any flat. (We can also edit inline/unnamed modifiers by the RAW 8-byte flat-id read from the record — no name needed — via `EditScalarFlatSafe`.)
- **Net:** "double HP/RAM" is NOT a single-flat edit. Target = the `ConstantStatModifier.value` (statType=Health/Memory, modifierType=Additive) inside the StatModifierGroup applied to `Character.Player_Puppet_Base`. Probe now walks `Character.Player_Puppet_Base` + `Modifiers.Health*` start records, recursing group→modifier and dumping inline/flat floats (range-tagged `<<HP?`/`<<RAM?`). Re-verify via `tools/ghidra/re-seeder-1..4.py`.

### F-040 — The RAM-cyberware pivot is also a dead end: Memory modifier `.value` is not an editable Float flat (2026-06-02)
- **Test (in-vivo, title screen, `IdentifyStatModifiers` + `TWEAKXL_MEMMOD_VERBOSE=1`, build 2.3.1):** scanned all 75 `Items.*MemoryBoost*/FastAccessMemory/MemoryReplenishment*_inlineN` candidate records via the game's own `GetFlat`. Result: **`scanned=75 matches=0 noStatType=66 otherStat=9`.**
- **66/75 are not StatModifiers at all** — `<rec>.statType` does not resolve as a flat (they are UIData / effectors / packages / status-effect refs, not `ConstantStatModifier`s).
- **The 9 that ARE stat modifiers all target `statType=0x d5a17fe2`** — a single memory-related stat that is **NOT** `BaseStats.Memory` (`0x4c58e91c`, confirmed live this run) nor `BaseStatPools.Player_Memory_Base` (`0x41af9eeb`). Its name is not in `record_names.txt` and was not recovered by CRC32 brute-force; likely a cyberware-memory sub-stat folded into Memory by a Combined/Curve modifier.
- **DECISIVE NEGATIVE — none of the 9 has an editable Float `.value`:** every one reads `value=(non-float)` via the game's `GetFlat` (the `.value` flat resolves but its FlatValue vtable ≠ Float vtable `0x106e925f0`). So the bonus magnitude is **inline on the materialized record / non-scalar**, exactly as F-039 found for the player's static modifiers — NOT a named Float scalar our `EditScalarFlatSafe`-by-name path can touch.
- **Hash algorithm re-confirmed:** `MakeCompactId` = plain zlib `CRC32(name)` — `CRC32("BaseStats.Memory")=0x4c58e91c`, `CRC32("BaseStatPools.Player_Memory_Base")=0x41af9eeb`, both match live record hashes.
- **Conclusion:** cyberdeck RAM is not reachable by a scalar-flat edit at either layer (base = computed per-entity F-039; cyberware bonus = inline/non-Float modifier value). Delivering a *visible, verified* RAM change needs either inline/array writes + the live-container oracle (walled by F-038) or deeper entity-init RE — beyond v1.0's scalar applicator.
- **For a guaranteed visible v1.0 win (the project goal "Windows TweakXL flat mods visibly work on macOS"):** pivot to an editable stored scalar — **`Price.*` records** (253 exist: `.buyPrice`/`.sellPrice`/`.pricePerPoint`, `gamedataPrice_Record`) read live by vendors. Edit ×2 via the verified path + confirm with the game's own `GetFlat`; user sees the changed price at a vendor.
- **How to re-verify:** `TWEAKXL_ID_MEM_MODS=1 TWEAKXL_MEMMOD_VERBOSE=1`; grep `[mem-mod] DIAG`/`done`.
- **Invalidates:** the actionable premise of the F-039 "Net" line and the RAM-ID next_action (target a stored Memory `ConstantStatModifier.value`) — no such editable Float exists.
