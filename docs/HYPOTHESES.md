...
- **Status:** **resolved-fact (2026-07-04)** — CONFIRMED on build 2.3.1 by `h004_probe.cpp` (T-005 finally run). `mach_vm_protect`→KERN_PROTECTION_FAILURE, `mprotect`→EPERM, on the untouched binary AND across four distinct ad-hoc re-sign entitlement configurations (see F-053). Also tested and refuted: H-012's proposed fix. See **F-053**.
- **Owner:** researcher
...
- **Status:** **resolved-fact (2026-05-30)** — baseHash resolved (F-033, vtable+0x118), factory-build validated in-vivo (no crash, faithful), Assign confirmed at type-vtable+0x50 (F-034), UpdateRecord implemented + wired into the applicator (E2E `records-updated=1/1`, no crash). See **F-033 / F-034**.
- **Owner:** researcher (baseHash table) → engineer (impl)

### H-012: Re-signing the macOS binary with `allow-unsigned-executable-memory` + `allow-jit` (on top of the entitlements already present) may unlock inline/JIT-based function hooking — a candidate fix for FA-001

- **Proposed:** 2026-07-04 by researcher, from external deep-research (`/deep-research` workflow) into the community macOS modding landscape, followed by manual code-reading of `github.com/memaxo/RED4ext-macos` (a ~4-day-old, single-author, unmerged community fork of RED4ext attempting Apple Silicon support).
- **Why we think this:** their `docs/MACOS_CODE_SIGNING.md` states Cyberpunk 2077's macOS binary already has `com.apple.security.cs.allow-dyld-environment-variables` and `com.apple.security.cs.disable-library-validation` (confirmed matching this project's own baseline — see AGENTS.md Scope & Intent) but is **missing** `com.apple.security.cs.allow-unsigned-executable-memory` and `com.apple.security.cs.allow-jit`. They ship a real, working re-signing script (`scripts/macos_resign_for_hooks.sh` + `macos_resign_backup.sh`/`macos_resign_restore.sh`) that signs nested code inside-out (never `codesign --deep`, which Apple explicitly warns against) and applies entitlements only to the main executable + app bundle — sound methodology, independently readable in their repo.
- **Independent cross-validation (raises confidence in their RE, not their hooking claims):** two of their hardcoded hook-target addresses (`CBaseEngine::LoadScripts` @ offset `0x3D9A03C`, `CBaseEngine::InitScripts` @ `0x3D8C1A0`) land within ~20 bytes of this project's own independently-derived **F-048** addresses (`FUN_103d9a028`, `FUN_103d8c188`) — two separate RE efforts, different methods, same functions.
- **CRITICAL CAVEAT — their own port does NOT demonstrate a working general hook API, so this is a narrower/riskier hypothesis than it first looks:** their `DetourAttach` (the function any real RED4ext plugin calls at runtime to install a hook) is a literal no-op in shipped Frida-Gadget mode — it logs and returns success without patching anything. The only hooks that fire are 9 addresses hardcoded into a static Frida JS file at build time; the `rpc.exports` C++↔JS bridge exposes only telemetry (hook count/stats/log level), with no mechanism for a plugin to request a hook dynamically. Their own TweakDB-specific hook offsets in that same file are `offset: 0x0, enabled: false` (never resolved). Their claim "TweakXL: tested/working" almost certainly means the plugin loads without crashing, not that TweakXL's actual gameplay-hooking mechanism fires. Additionally, the repo's own docs contain an unresolved internal contradiction: `HOOKING_ALTERNATIVES.md` claims Frida's JIT trampolines bypass code signing *without* re-signing the binary, while `MACOS_CODE_SIGNING.md` (and the real script) says hooks require re-signing with the two extra entitlements above — these two documents were not reconciled by the (single) author.
- **What this hypothesis actually claims (narrowed, testable):** re-signing a BACKUP copy of the game binary with `allow-unsigned-executable-memory` + `allow-jit` may enable either (a) Frida-based hooking to work as they describe, or (b) genuine native/inline `__TEXT` patching (defeating FA-001 directly) — neither has been demonstrated by them or by us. This is a lead worth a cheap, isolated experiment, not a solution to adopt.
- **How to test:** back up the game bundle (their `macos_resign_backup.sh` logic is a reasonable reference, re-derive rather than run their script verbatim), re-sign with the two additional entitlements only on the main executable (never `--deep`), then attempt a minimal native inline hook (not via Frida) on a known-safe target and see if `mach_vm_protect`/`mprotect` on an existing `__TEXT` page now succeeds where FA-001 previously failed. Verify game still launches and passes Steam's integrity check path; keep the restore script/backup path available.
- **Risk note:** re-signing the game binary is a materially more invasive action than anything this project has done to date (redscript, F-050–052, required zero entitlement changes) — get explicit user sign-off before performing it, even against a backup copy, per the project's action-risk discipline.
- **Status:** **resolved-fact, REFUTED (2026-07-04)** — user approved the test; performed against isolated copies only (never the live install). `allow-unsigned-executable-memory` + `allow-jit` added to a minimal, non-restricted entitlement base: `mach_vm_protect`/`mprotect` on `__TEXT` still fail identically to the untouched baseline (KERN_PROTECTION_FAILURE / EPERM). Extended the test to also cover `disable-executable-page-protection` (this project's own separate claim) and `get-task-allow` — both also refuted, same failure. See **F-053** for the full 6-variant methodology, including an incidental but important finding: preserving CDPR's *restricted* provisioned entitlements under an ad-hoc signature causes an instant SIGKILL, unrelated to the `__TEXT` question. The remaining untested lever (not ruled out by this experiment) is a real, non-ad-hoc Apple Developer ID signature.
- **Owner:** researcher
